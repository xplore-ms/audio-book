...
@router.get("/")
def list_voices(user=Depends(get_current_user), ttl: int = Query(900, description="Signed URL TTL in seconds")):
    docs = list(voices_collection.find({}, {"supabase_path": 1, "voice_name": 1, "display_name": 1, "language_codes": 1, "ssml_gender": 1, "sample_text": 1}))
...
def get_voice_signed_url(voice_id: str, user=Depends(get_current_user), ttl: int = Query(900)):
    try:
