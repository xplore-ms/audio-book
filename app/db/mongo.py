...
sync_client = MongoClient(config.MONGO_URL, **COMMON_OPTS)
sync_db = sync_client[config.MONGO_DB]
...
async_client = AsyncIOMotorClient(config.MONGO_URL, **COMMON_OPTS)
async_db = async_client[config.MONGO_DB]
...
def ensure_indexes():
    ...
    # Job tasks
    job_tasks.create_index([("job_id", ASCENDING), ("processing_id", ASCENDING), ("page", ASCENDING)], unique=True, background=True)
    # Make celery_task_id index partial to avoid unique constraint on missing/null values
    try:
        job_tasks.create_index([("celery_task_id", ASCENDING)], unique=True, partialFilterExpression={"celery_task_id": {"$type": "string"}}, background=True)
    ...
    job_tasks.create_index([("job_id", ASCENDING), ("state", ASCENDING)], background=True)
    ...
    # Payments
    payments_collection.create_index([("reference", ASCENDING)], unique=True, background=True)
    ...
    try:
        voices_collection.create_index([("voice_name", ASCENDING)], unique=True, partialFilterExpression={"voice_name": {"$type": "string"}}, background=True)
    except OperationFailure as e:
        if e.code == 86:
...
async def ensure_indexes_async():
    ...
    await jobs_collection_async.create_index([("job_id", ASCENDING)], unique=True, background=True)
    ...
    await jobs_collection_async.create_index([("created_at", DESCENDING)], background=True)

    await job_tasks_async.create_index([("job_id", ASCENDING), ("processing_id", ASCENDING), ("page", ASCENDING)], unique=True, background=True)
    try:
        await job_tasks_async.create_index([("celery_task_id", ASCENDING)], unique=True, background=True, partialFilterExpression={"celery_task_id": {"$type": "string"}})
    except OperationFailure as e:
        if e.code == 86:
    ...
    await job_tasks_async.create_index([("job_id", ASCENDING), ("state", ASCENDING)], background=True)
    await job_tasks_async.create_index([("created_at", DESCENDING)], background=True)

    await users_collection_async.create_index([("email", ASCENDING)], unique=True, background=True)

    await payments_collection_async.create_index([("reference", ASCENDING)], unique=True, background=True)
    await payments_collection_async.create_index([("user_id", ASCENDING)], background=True)
    await payments_collection_async.create_index([("status", ASCENDING)], background=True)
    await payments_collection_async.create_index([("created_at", DESCENDING)], background=True)

    try:
        await voices_collection_async.create_index([("voice_name", ASCENDING)], unique=True, background=True, partialFilterExpression={"voice_name": {"$type": "string"}})
    except OperationFailure as e:
        if e.code == 86:
    ...
    await voices_collection_async.create_index([("created_at", DESCENDING)], background=True)
